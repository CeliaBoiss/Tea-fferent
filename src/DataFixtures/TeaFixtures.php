...
class TeaFixtures extends Fixture implements DependentFixtureInterface
{
    ...
    {
        foreach (self::TEAS as $name => $data) {
            $tea = new Tea();
            $tea->setName($name);
            $tea->setDescription($data['description']);
            $tea->setPrice($data['price']);
            $tea->setPhoto($data['photo']);
            $tea->setType($this->getReference($data['type']));
